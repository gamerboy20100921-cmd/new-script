--[[
    Anime Mob Hub - Optimized Version
...
local function NotificacaoNightMystic(titulo, mensagem)
    local success = pcall(function()
        ...
        local LogoID = "rbxassetid://78306014282581"
    ...
    if not success then
        warn("[Anime Mob] Erro ao exibir notificação")
    end
end

NotificacaoNightMystic("Anime Mob Hub", "Script carregado com sucesso!")
...
-- ========================================
-- Wait for game and character before initializing globals
...
QuestNeta = function()
	...
	local function LoadAnimeMobUILibrary()
    local urls = {
        "https://raw.githubusercontent.com/PlockScripts/Library-ui/refs/heads/main/Redzhubui",
        "https://raw.githubusercontent.com/PlockScripts/Library-ui/main/Redzhubui",
    }
    ...
    for _, url in ipairs(urls) do
        ...
        if okSource and type(source) == "string" and #source > 1000 then
            local loader, loadErr = loadstring(source)
            if loader then
                local okLib, lib = pcall(loader)
                if okLib and lib and type(lib.MakeWindow) == "function" then
                    return lib
                end
            else
                warn("[Anime Mob Hub] Erro loadstring UI:", loadErr)
            end
        end
    end

    error("[Anime Mob Hub] Não foi possível carregar a biblioteca da UI.")
end

	local Library = LoadAnimeMobUILibrary():MakeWindow({
    Title = "Anime Mob Hub",
    SubTitle = "by Rafael and animewaze",
    SaveFolder = "anime_mob_hub.json"
})
-- Criar ScreenGui
local screenGui = Instance.new("ScreenGui")
...
imageButton.Image = "rbxassetid://78306014282581"
...
uiStroke.Color = Color3.fromRGB(255, 215, 0) -- Magenta/Rosa forte
...
Discord:AddDiscordInvite({
    ...
    Logo = "rbxassetid://78306014282581",
    Invite = "https://discord.gg/Wk6rS8pZ3"
})
local credits = Discord:AddParagraph({
    Title = "Credits For Someone Peoples",
    Desc = ""
})
credits:SetDesc("Creator: Rafael and animewaze")
...
NotificacaoNightMystic("Anime Mob Hub", "✅ Todos os sistemas carregados!")
